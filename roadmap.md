...
4.  **Refactor `AutoPilot` and `Allocator`:**
    *   Make `AutoPilot` inherit from `PilotBase`.
    *   Make `Allocator` inherit from `AllocatorBase`.
    *   Replace the `if/elif` flight-mode logic with a **Strategy Pattern**, where each flight mode maps to a class implementing a `FlightModeStrategy` interface.
...
8.  **Unify the Logging Contract:** Ensure all refactored subsystems use the `@loggable` decorator exclusively. Remove all stray `print()` statements from the core simulation pipeline.
